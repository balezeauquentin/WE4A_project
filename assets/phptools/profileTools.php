...
function updateProfile($bdd, $username, $email, $profile_picture, $banner, $bio, $password, $current_profile_picture_path, $current_banner_path)
{
    ...
    try {
        if (!empty($password)) {
            ...
            $sql = "UPDATE users SET email = ?, profile_picture_path = ?, banner_path = ?, bio = ?, password = ? WHERE username = ?";
            $stmt = $bdd->prepare($sql);
            $stmt->execute([$email, $pp_path_db, $banner_path_db, $bio, $password, $username]);
        } else {
